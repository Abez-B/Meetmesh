...
const CURSOR_DIST     = 70;    // px — radius of cursor repulsion bubble
const CURSOR_STR      = 0.45;  // strength of cursor push per px overlap
...
export function SpringMeshPreview() {
  ...
  const sizeRef  = useRef({ w: 320, h: 280 });
  const readyRef = useRef(false);

  /* Fixed base angles, spread evenly */
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      for (let i = 0; i < N; i++) {
        ...
        /* Spring toward slowly-drifting target — gives organic "breathing" */
        const driftAngle = baseAngles[i] + Math.sin(t * 0.55 + i * 1.85) * DRIFT_ANGLE_AMP;
        const driftDist  = REST_DIST     + Math.sin(t * 0.38 + i * 2.60) * DRIFT_DIST_AMP;
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    mouseRef.current = {
      ...
      y: (e.clientY - rect.top)  / rect.height * h - h / 2,
    };
  };
  ...
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <g ref={gRef} transform="translate(160,140)">
        ...
        {/* ── Host node (stays at origin) ───────────────────────── */}
        <g style={{ cursor: 'default' }}>
          <circle r="28" fill="none" stroke="#FFD700" strokeWidth="3" opacity="0.9" />
          <circle r="23"
            fill="radial-gradient(circle, #FFE566, #FFD700)"
            style={{ fill: 'url(#smp-sun)' }}
          />
          <defs>
